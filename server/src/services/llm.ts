...
type Provider = "MOCK" | "OPENAI" | "DASHSCOPE";
...
export async function generateItinerary(input: PlanInput, opts: { provider: Provider; apiKey: string }) {
  if (opts.provider === "MOCK") {
    ...
    const pick = (i: number) => [tokyoSpots[(i*2)%tokyoSpots.length], tokyoSpots[(i*2+1)%tokyoSpots.length], tokyoSpots[(i*2+2)%tokyoSpots.length]];
    const makeDay = (i: number) => {
      const [m, a, e] = pick(i);
      const withDetails = (slot: string, spot: any) => ({
        timeSlot: slot,
        ...spot,
  ...
  if (opts.provider === "DASHSCOPE") {
    ...
    if (!resp.ok) throw new Error(`DashScope error: ${resp.status}`);
    const json = await resp.json() as any;
